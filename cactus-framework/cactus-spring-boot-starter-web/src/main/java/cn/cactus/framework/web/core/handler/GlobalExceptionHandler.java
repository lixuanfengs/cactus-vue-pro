package cn.cactus.framework.web.core.handler;

import cn.cactus.framework.apilog.core.service.ApiErrorLog;
...
import cn.cactus.framework.common.util.monitor.TracerUtils;
...
import static cn.cactus.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 cactus-module-report - 表结构未导入][参考 https://cactusli.net/report/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 cactus-module-report - 表结构未导入][参考 https://cactusli.net/report/ 开启]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 cactus-module-bpm - 表结构未导入][参考 https://cactusli.net/bpm/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 cactus-module-bpm - 表结构未导入][参考 https://cactusli.net/bpm/ 开启]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 cactus-module-mp - 表结构未导入][参考 https://cactusli.net/mp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 cactus-module-mp - 表结构未导入][参考 https://cactusli.net/mp/build/ 开启]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 cactus-module-mall - 已禁用][参考 https://cactusli.net/mall/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 cactus-module-mall - 已禁用][参考 https://cactusli.net/mall/build/ 开启]");
        }
        // 5. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 cactus-module-pay - 表结构未导入][参考 https://cactusli.net/pay/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 cactus-module-pay - 表结构未导入][参考 https://cactusli.net/pay/build/ 开启]");
        }
        return null;
    }
